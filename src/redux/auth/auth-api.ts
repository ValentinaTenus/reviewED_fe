	...
	endpoints: (builder) => ({
		getLoginUrl: builder.query<AuthResponseDto, undefined>({
			query: () => {
				return {
					...
					url: `${BASE_URI}${authApiPath.LOGIN}?redirect_uri=${REDIRECT_URI}`,
				};
			},
		}),
		login: builder.query<LoginResponseDto, { code: string; state: string }>({
			query: ({ code, state }) => {
				return {
					method: HttpMethods.GET,
					url: `${BASE_URI}${authApiPath.LINKEDIN_CALLBACK}?code=${code}&state=${state}&redirect_uri=${REDIRECT_URI}`,
				};
			},
		}),
		logOut: builder.query<string, LogOutRequestDto>({
			query: (refresh) => {
				return {
					...
					url: `${BASE_URI}${authApiPath.LOGOUT}`,
				};
			},
		}),
	}),
